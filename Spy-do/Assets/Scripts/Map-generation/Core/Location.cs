        ...
        class Location
        {
            ...
            private Room[] generateLocationRooms(SLocationOfRoomsInformation slori)
            {
                ...
                while (i < NumberOfLocationRooms)
                {
                    ...
                    if ((randomRoom == 0) && (slori.isGym) && (localSLORI.gymQuantity <= slori.gymQuantity)
                    ) //Checks if such a room is included in location generation
                    {
                        generatedRooms[room] = new Gym(Random.Range(slori.gymMinHeightY, slori.gymMaxHeightY),
                            Random.Range(slori.gymMinLengthX, slori.maxLocationLengthX));
                        room++;
                        localSLORI.gymQuantity--;
                        i++;
                    }
                    else if ((randomRoom == 1) && (slori.isOffice) &&
                             (localSLORI.officeQuantity <= slori.officeQuantity)
                    ) //Checks if such a room is included in location generation
                    {
                        generatedRooms[room] = new Office(Random.Range(slori.officeMinHeightY, slori.officeMaxHeightY),
                            Random.Range(slori.officeMinLengthX, slori.officeMaxLengthX));
                        room++;
                        localSLORI.officeQuantity--;
                        i++;
                    }
                    else if ((randomRoom == 2) && (slori.isEmptySpace) &&
                             (localSLORI.emptySpaceQuantity <= slori.emptySpaceQuantity))
                    {
                        generatedRooms[room] =
                            new EmptySpace(Random.Range(slori.emptySpaceMinHeightY, slori.emptySpaceMaxHeightY),
                                Random.Range(slori.emptySpaceMinLengthX, slori.emptySpaceMaxLengthX));
                        room++;
            ...
            private string[,,] createLocationObjectMap(SLocationOfRoomsInformation slori)
            {
                string[,,]
                    objectMap =
                        createLocationDefaultRoom(); //new string[LocationLayersZ, LocationHeightY, LocationLengthX];
                ...
                for (int idHeightY = 0;
                    idHeightY < LocationIdMapHeightY;
                    idHeightY++) //goes through ID MAP in Y dimension 
                {
                    for (int idLengthX = 0;
                        idLengthX < LocationIdMapLengthX;
                        idLengthX++) //goes through ID MAP in X dimension 
                    {
                        if ((LocationIdMap[idHeightY, idLengthX] == 1) && (roomNumber < NumberOfLocationRooms)
                        ) //creates room if default value (1)
                        {
                            ...
                            //creates room
                            for (int layerZ = 0;
                                layerZ < LocationRooms[roomNumber].RoomLayers.Count;
                                layerZ++) //goes through layers (or Z dimension) in objectMap
                            {
                                for (int omY = 0;
                                    omY < LocationRooms[roomNumber].RoomLayers[layerZ].LayerObjectMap.GetLength(0);
                                    omY++) //goes through Y dimension in objectMap
                                {
                                    for (int omX = 0;
                                        omX < LocationRooms[roomNumber].RoomLayers[layerZ].LayerObjectMap.GetLength(1);
                                        omX++) //goes through X dimension in objectMap 
                                    {
                                        objectMap[layerZ, omY + spacingY, omX + spacingX] = LocationRooms[roomNumber]
                                            .RoomLayers[layerZ].LayerObjectMap[omY, omX];
                                    }
                                }
                            }
                ...
                {
                    objectMap[objectMap.GetLength(0) - 1, 0, x] =
                        "GRBottomWall"; //SETS BUTTOM WALL TO PREVENT BUGS WITH EMPTY SPACE (has to fixed in the future)
                }
