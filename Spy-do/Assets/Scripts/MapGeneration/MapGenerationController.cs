...
{   
    ...
    void Start()
    {
        createRoomLayer(room.GetRoomObjectMap(), room.GetRoomLengthX(), room.GetRoomHeightY());
        createRoomLayer(room.GetRoomLayer().getLayerObjectMap(), room.GetRoomLayer().GetLayerLengthX(), room.GetRoomLayer().GetLayerHeightY());
        //room.Test();
    }

    private void createRoomLayer(string[,] objectMap, int lengthX, int heightY) 
    {
...
/*
private void createRoom(string[,] objectMap, int heightY, int lengthX) 
    {
        for (int y = 0; y < room.GetRoomHeightY(); y++) 
        {
            for (int x = 0; x < room.GetRoomLengthX(); x++) 
            {
                switch (room.GetRoomObjectMap()[y, x]) 
                {
                    case "floor":
                        Instantiate(floor, new Vector2(x, y), Quaternion.identity);
                        break;
